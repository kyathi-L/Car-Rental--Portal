...
-- Add foreign key constraint to tblbrands table
ALTER TABLE `tblbrands`
  ADD COLUMN `aid` INT(11) NOT NULL,
  ADD FOREIGN KEY (`aid`) REFERENCES `admin`(`id`);
-- Add foreign key constraint to tblpages table
ALTER TABLE `tblpages`
  ADD COLUMN `aid` INT(11) NOT NULL,
  ADD FOREIGN KEY (`aid`) REFERENCES `admin`(`id`);

-- Add foreign key constraint to tblcontactusinfo table
ALTER TABLE `tblcontactusinfo`
  ADD COLUMN `aid` INT(11) NOT NULL,
  ADD FOREIGN KEY (`aid`) REFERENCES `admin`(`id`);

-- Add foreign key constraint to tblvehicles table
ALTER TABLE `tblvehicles`
  ADD COLUMN `brand` INT(11) NOT NULL,
  ADD FOREIGN KEY (`brand`) REFERENCES `tblbrands`(`id`);
-- Add foreign key constraint to tblbooking table
ALTER TABLE `tblbooking`
  ADD COLUMN `uid` INT(11) NOT NULL,
  ADD FOREIGN KEY (`uid`) REFERENCES `tblusers`(`id`);
-- Add foreign key constraint to tbltestimonial table
ALTER TABLE `tbltestimonial`
  ADD COLUMN `uid` INT(11) NOT NULL,
  ADD FOREIGN KEY (`uid`) REFERENCES `tblusers`(`id`);
-- Add foreign key constraint to tblcontactusquery table
ALTER TABLE `tblcontactusquery`
  ADD COLUMN `uid` INT(11) NOT NULL,
  ADD FOREIGN KEY (`uid`) REFERENCES `tblusers`(`id`);
-- Add foreign key constraint to tblsubscribers table
ALTER TABLE `tblsubscribers`
  ADD COLUMN `uid` INT(11) NOT NULL,
  ADD FOREIGN KEY (`uid`) REFERENCES `tblusers`(`id`);

-- Identify orphaned values
SELECT * FROM tblbrands WHERE aid NOT IN (SELECT id FROM admin);

-- Update orphaned values to valid id values
UPDATE tblbrands SET aid = NULL WHERE aid NOT IN (SELECT id FROM admin);

-- Delete rows containing orphaned values
DELETE FROM tblbrands WHERE aid NOT IN (SELECT id FROM admin);
